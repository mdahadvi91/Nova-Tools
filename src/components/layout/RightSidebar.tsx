...
export const RightSidebar: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end animate-fade-in">
      ...
      <div className="relative w-80 sm:w-96 max-w-[90vw] h-full liquid-glass border-s border-slate-200 dark:border-white/10 shadow-2xl flex flex-col justify-between z-10 overflow-hidden">
        {/* Header */}
        <div className="p-4 sm:p-5 border-b border-slate-200 dark:border-white/10 flex items-center justify-between">
          <div>
            <h2 className="text-base font-bold text-slate-900 dark:text-white tracking-tight">Preferences & Settings</h2>
            <p className="text-[11px] text-slate-500 dark:text-slate-400">Language, Theme & Configuration</p>
          </div>
          <button
            type="button"
            onClick={closeAllSidebars}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
              ...
              {languages.map((item) => (
                <button
                  ...
                  className={`flex items-center justify-between px-3.5 py-2.5 rounded-xl text-xs sm:text-sm font-medium transition-all ${
                    language === item.code
                      ? 'bg-emerald-500/15 dark:bg-emerald-500/20 text-emerald-800 dark:text-emerald-300 border border-emerald-500/40 font-semibold'
                      : 'bg-slate-100/80 hover:bg-slate-200/80 dark:bg-white/5 dark:hover:bg-white/10 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white border border-slate-200/80 dark:border-white/5'
                  }`}
                >
          ...
          {favoriteTools.length > 0 && (
            <div className="space-y-2">
              <div className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-amber-500 dark:text-amber-400">
              ...
              <div className="space-y-1">
                {favoriteTools.slice(0, 5).map((tool) => (
                  <button
                    ...
                    className="w-full text-start px-3 py-2 rounded-xl bg-slate-100/80 hover:bg-slate-200/80 dark:bg-white/5 dark:hover:bg-white/10 text-xs text-slate-700 dark:text-slate-200 hover:text-slate-900 dark:hover:text-white flex items-center justify-between"
                  >
          ...
          <div className="space-y-1.5 pt-2 border-t border-slate-200 dark:border-white/10">
            ...
            <button
              ...
              className="w-full flex items-center gap-2 px-2 py-1.5 text-xs text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200"
            >
            ...
            <button
              ...
              className="w-full flex items-center gap-2 px-2 py-1.5 text-xs text-emerald-700 dark:text-emerald-400 hover:text-emerald-800 dark:hover:text-emerald-300 font-medium"
            >
              <ShieldCheck className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" />
        ...
        <div className="p-3.5 border-t border-slate-200 dark:border-white/10 bg-slate-100/50 dark:bg-black/20 text-center">
          <div className="inline-flex items-center gap-1.5 text-[11px] text-emerald-700 dark:text-emerald-400 font-medium">
